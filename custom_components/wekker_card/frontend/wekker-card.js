...
class WekkerCard extends HTMLElement {
  ...
  setConfig(config) {
    this._config = {
      ...
      enabled_entity: "switch.wekker_card_enabled",
      status_entity: "sensor.wekker_card_status",
      snooze_until_entity: "sensor.wekker_card_snooze_until",
      speaker_select_entity: "select.wekker_card_speaker",
      weekday_time_entity: "time.wekker_card_weekday",
      weekend_time_entity: "time.wekker_card_weekend",
      media_uri_entity: "text.wekker_card_media_uri",
      media_type_entity: "select.wekker_card_media_type",
      favorite_select_entity: "select.wekker_card_favorite",
      ...
      light_brightness_entity: "number.wekker_card_light_brightness",
      snooze_button: "button.wekker_card_snooze",
      stop_button: "button.wekker_card_stop",
      refresh_button: "button.wekker_card_refresh",
      ...config,
    };
  ...
  _select(entityId, label) {
    ...
    return `
      <label class="field field-wide">
        ...
        <select data-entity="${this._escape(entityId)}" data-domain="select" data-service="select_option" data-field="option">
  ...
  _time(entityId, label) {
    ...
    return `
      <label class="field">
        <span>${this._escape(label)}</span>
        <input type="time" value="${this._escape(value)}" data-entity="${this._escape(entityId)}" data-domain="time" data-service="set_value" data-field="time">
      </label>`;
  }
  ...
  _number(entityId, label, suffix) {
    ...
    return `
      <label class="field field-wide range-field">
        ...
        <input type="range" min="${min}" max="${max}" step="${step}" value="${value}" data-entity="${this._escape(entityId)}" data-domain="number" data-service="set_value" data-field="value">
      </label>`;
  }
      ...
      <section class="settings-panel">
        ...
        <div class="form-grid">
          ...
          <label class="field field-wide">
            ...
            <input type="text" value="${this._escape(this._state(c.media_uri_entity, ""))}" data-entity="${this._escape(c.media_uri_entity)}" data-domain="text" data-service="set_value" data-field="value">
          </label>
  ...
  _handleClick(event) {
    ...
    if (action === "toggle-alarm") this._call("switch", "toggle", this._config.enabled_entity);
    if (action === "toggle-light") this._call("switch", "toggle", this._config.light_enabled_entity);
    if (action === "snooze") this._call("button", "press", this._config.snooze_button);
    if (action === "stop") this._call("button", "press", this._config.stop_button);
    if (action === "refresh-lists") this._call("button", "press", this._config.refresh_button);
  }
  ...
  _handleChange(event) {
    ...
    if (!control) return;
    const data = {};
    data[control.dataset.field] = control.dataset.domain === "number" ? Number(control.value) : control.value;
    if (control.dataset.entity === this._config.media_uri_entity) {
      this._call("text", "set_value", this._config.media_uri_entity, data);
      return;
    }
